...
bool Board::underAttack(Color opp, int x, int y)
{
	...
	if (opp == BLK) {
		if ((this->getTileAtPoint(x + 1, y + 1)->getColor() == opp && this->getTileAtPoint(x + 1, y + 1)->getType() == P) || (this->getTileAtPoint(x - 1, y + 1)->getColor() == opp && this->getTileAtPoint(x - 1, y + 1)->getType() == P)) {
			return true;
		}
	}
	if (opp == WHT) {
