...
//first item in array is how many tiles in array
//a 2d array represented using a 1d block of memory
int* getTilesBetween(Position tOne, Position tTwo) {

	int height = max(std::abs(tOne.x - tTwo.x), std::abs(tOne.y - tTwo.y));

	int* arr = new int[1 + height * 2];
	arr[0] = 0;
	int x = 1;

	if (tOne.y == tTwo.y) {

		int startCol = min(tOne.x, tTwo.x);
		int endCol = max(tOne.x, tTwo.x);
	...
	if (tOne.x == tTwo.x) {
		...
		int startRow = min(tOne.y, tTwo.y);
		int endRow = max(tOne.y, tTwo.y);
	...
	if (abs(tOne.x - tTwo.x) == abs(tOne.y - tTwo.y)) {
		
		int xDir = (tOne.x < tTwo.x) ? 1 : -1;
		int yDir = (tOne.y < tTwo.y) ? 1 : -1;

		arr[0] = abs(tOne.x - tTwo.x) - 1;

		int xCoord = tOne.x + xDir;
		int yCoord = tOne.y + yDir;

		for (int i = xCoord; i < tTwo.x; i++) {
...
bool isCheckmate(Board board) {
	...
	if (board.underAttack(opp, kingX, kingY, xAttacker, yAttacker) && !board.underAttack(self, xAttacker, yAttacker)) {		
		...
		//if can be blocked by own pieces not checkmate, but can't be a move that reveals another check
		int* arr = getTilesBetween( { kingX, kingY }, { xAttacker, yAttacker } );
		...
		for (int i = 0; i < arr[0]; i++) {
			...
			if (board.underAttack(self, arr[indx], arr[indx + 1], xDefender, yDefender, true)) {


				str from = "";
				from += char(xDefender) + 64;
				from += char(yDefender) + 48;

				str to = "";
				to += char(arr[indx]) + 64;
				to += char(arr[indx+1]) + 48;


				if (!stillUnderCheck(board, from, to, self, opp)) {
					delete arr;
					return false;
				}
			}
